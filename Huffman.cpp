...
int main(int argc, char** argv){
    ...
    */
    ...
    for(int i=0;i<diccionario.size();i++){//Relleno Lista de Huffman Canonico con lista ordenada
        ...
        huffmanCanonico[i][1]=contadorCodigo;
        contadorCodigo = "1"+contadorCodigo;
        listaOrdenada.pop();
        cout<<huffmanCanonico[i][0]<<" "<<huffmanCanonico[i][1]<<endl;
    }
    contadorCodigo.pop_back();
    contadorCodigo.pop_back();
    huffmanCanonico[diccionario.size()-1][1]=contadorCodigo;
    ...
    string output = "";
    for(int i=0; i<cadena.length();i++){
        output = output + CanoHuffman[cadena[i]];
    }
    ...
    for(int i=0;i < diccionario.size();i++){
        decoder[huffmanCanonico[i][1]]=char(huffmanCanonico[i][0][0]);
    }
